...
if Modules == nil then
	...
	--Pets
	function StdModule.darPetStarter(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		...
		if message=="cat" then
			...
			if tonumber(petNumber) ~= nil and tonumber(petNumber) <= table.maxn(PETS.IDENTIFICATION) then
				player:doRemovePet()
				player:doResetPet()
				if player:doAddPet(tonumber(petNumber)) then
